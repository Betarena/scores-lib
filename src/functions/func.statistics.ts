 ...
 */
export async function ST_F_get_target_historic_fixtures (
  initGrapQLClient: GraphQLClient,
  seasonIdsArr: number[]
): Promise < B_H_ST_SJ_HF[] > {
  ...
  while (true) {
    ...
    const response: B_H_ST_Q = await initGrapQLClient.request (
      B_C_STAT_F_Q_D0,
      VARIABLES
    );
 ...
 * @returns Promise < Map <number, B_H_ST_SJ_HF> >
 */
export async function ST_F_generate_historic_fixtures_map (
  h_fixtures_arr: B_H_ST_SJ_HF[]
): Promise < Map <number, B_H_ST_SJ_HF> > {

  const historic_fixtures_map = new Map <number, B_H_ST_SJ_HF>()
 ...
 * @param {Map <number, B_H_ST_SJ_HF>} historic_fixtures_map 
 * @returns Promise < ST_Fixture[] >
 */
export async function ST_F_data_main (
  historic_fixtures_map: Map <number, B_H_ST_SJ_HF>
): Promise < ST_Fixture[] > {

  const cache_data_arr: ST_Fixture[] = []
  ...
  for (const [key, value] of historic_fixtures_map.entries()) {
    ...
    const home_team_obj: ST_Incident_Team = {
      team_name: home_team_name || null,
      team_logo: home_team_logo || null,
      team_id: home_team_id
    }

    const away_team_obj: ST_Incident_Team = {
    ...
    const fixture_object: ST_Fixture = {
      id:      fixture_id || null,
      status:  status,
      home:    home_team_obj,
      away:    away_team_obj,
 ...
 * @returns Promise < B_H_ST_SJ_HF[] >
 */
export async function ST_F_get_target_fixture(
  initGrapQLClient: GraphQLClient,
	fixture_id: number
): Promise < B_H_ST_SJ_HF[] > {
 ...
 * translation data for the Statistics Fixtures (widget);
 ...
 * @returns Promise < B_H_ST_Q >
 */
export async function ST_F_get_widget_translations (
  initGrapQLClient: GraphQLClient,
  langArray: string[]
): Promise < B_H_ST_Q > {
 ...
 * Statistics Fixtures (widget);
 * NOTE: can generate for a SINGLE or MULTI langauges, depending 
 * on the ones that are supplied;
 * @version 3.0
 * @param {B_H_ST_Q} data - data recieved from the Hasura DB
 * @param {string[]} langArray - array of target languages
 * @returns Promise < Map <string, B_ST_T> >
 */
export async function ST_F_translations_main (
  data: B_H_ST_Q,
  langArray: string[]
): Promise < Map <string, B_ST_T> > {
  
  const fix_odds_translation_map = new Map <string, B_ST_T> ()
  ...
  for (const lang_ of langArray) {
    ...
    const objectFixOdds = data.scores_fixture_stats_translations
      .find(({ lang }) => 
        lang === lang_
      )
